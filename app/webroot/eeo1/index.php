...
function login() {
    $state = md5(( double ) microtime() * 10000);
    $_SESSION['wx_state'] = $state;
    $url = "https://open.weixin.qq.com/connect/oauth2/authorize?appid=".WX_APP_ID."&redirect_uri=".WX_REDIRECT_URI."&response_type=code&scope=snsapi_base&state=" . $state . "#wechat_redirect";
    header('location:' . $url);
}
...
$(function(){ 
	...
	$(".Q14").click(function(){
		...
		{
			setTimeout("DisplayTopic('.Topic17')", 0 );
			setTimeout("DisplayTopic('.Topic18')", 500 );			
			setTimeout("DisplayTopic('.Submission')", 800 );
		}
